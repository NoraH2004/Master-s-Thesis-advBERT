...
While in a traditional neural network, all inputs and outputs are independent of each other, a RNN is recurrent, as it uses previous outputs as additional inputs for prediction. The loops within the hidden layer's neurons, illustrated in ~\ref{fig:RNN}, allow the model to store information about previous input sequences, called a sequencial momery. 
In other words, it performs the same task for every sample of a sequence of input data, while the forwarded output being depended on the previous computations.
...
To better illustrate our proceedings, we describe it using the proposed framework by Morris et al.:
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
original_sentence: "I have not have any problems."
