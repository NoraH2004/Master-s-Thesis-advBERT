...
\draw  [fill={rgb, 255:red, 248; green, 206; blue, 28 }  ,fill opacity=1 ] (218.07,206.97) .. controls (218.07,201.46) and (222.53,197) .. (228.03,197) .. controls (233.54,197) and (238,201.46) .. (238,206.97) .. controls (238,212.47) and (233.54,216.93) .. (228.03,216.93) .. controls (222.53,216.93) and (218.07,212.47) .. (218.07,206.97) -- cycle ;
%Shape: Circle [id:dp9175972472468675] 
\draw  [fill={rgb, 255:red, 74; green, 144; blue, 226 }  ,fill opacity=1 ] (337.73,205.63) .. controls (337.83,211.13) and (333.44,215.67) .. (327.94,215.76) .. controls (322.43,215.85) and (317.9,211.46) .. (317.8,205.96) .. controls (317.71,200.46) and (322.1,195.92) .. (327.6,195.83) .. controls (333.11,195.74) and (337.64,200.12) .. (337.73,205.63) -- cycle ;
...
% Text Node
\draw (89,122.4) node [anchor=north west][inner sep=0.75pt]    {$...$};
% Text Node
\draw (87,149.4) node [anchor=north west][inner sep=0.75pt]    {$x_{n}$};
% Text Node
\draw (87,220.8) node [anchor=north west][inner sep=0.75pt]  [font=\scriptsize]  {${\textstyle Input\ Layer\ l_{1}}$};
% Text Node
\draw (168,221.8) node [anchor=north west][inner sep=0.75pt]  [font=\scriptsize]  {${Hidden\ Layers\ l_{2} \ ...\ l_{5}}$};
% Text Node
\draw (293,219.8) node [anchor=north west][inner sep=0.75pt]  [font=\scriptsize]  {$Output\ Layer\ l_{6}$};
% Text Node
\draw (356,70.4) node [anchor=north west][inner sep=0.75pt]    {$y_{0}$};
% Text Node
\draw (358,104.4) node [anchor=north west][inner sep=0.75pt]    {$...$};
% Text Node
...
% Text Node
\draw (152,254.4) node [anchor=north west][inner sep=0.75pt]  [font=\footnotesize]  {${\textstyle(x,y)}$};
% Text Node
\draw (152,271.4) node [anchor=north west][inner sep=0.75pt]  [font=\footnotesize]  {$l$};
% Text Node
\draw (193.12,255.2) node [anchor=north west][inner sep=0.75pt]  [font=\footnotesize]  {${\textstyle ( input,\ output)}$};
% Text Node
\draw (193.12,271.2) node [anchor=north west][inner sep=0.75pt]  [font=\footnotesize]  {$layer$};
% Text Node
\draw (152,285.4) node [anchor=north west][inner sep=0.75pt]  [font=\footnotesize]  {$a$};
% Text Node
\draw (192.12,285.2) node [anchor=north west][inner sep=0.75pt]  [font=\footnotesize]  {$activation\ value$};
% Text Node
\draw (152,299.33) node [anchor=north west][inner sep=0.75pt]  [font=\footnotesize]  {$w$};
% Text Node
\draw (193.12,299.2) node [anchor=north west][inner sep=0.75pt]  [font=\footnotesize]  {$weight$};
...
The self-attention based Transformer model is not recurrent, hence the output is not fed back as input like in RNNs but uses its attention to "look back" to the crucial piece of information. Text sequences are not memorized or stored completely but rather the parts, which are considered relevant. This approach allows the Transformer to outperform RNNs and LSTM networks and makes the model more parallelizable, resulting in enhanced computational performance. Transformer models proved to improve the state-of-the-art approaches in the field of NLP \cite{vaswani2017attention}. 

Famous Transformer models are ERNIE \cite{zhang2019ernie}, BERT \cite{devlin2018bert}, XLnet \cite{yang2019xlnet}, RoBERTa \cite{liu2019roberta}, and DistilBERT \cite{sanh2019distilbert}. More detailed information about the Transformer Model BERT is provided in Section~\ref{sec:bert}.

\subsubsection{Tasks in NLP}
\label{sec:tasks_in_nLP}

Next to machine translation, there is a broad scope of tasks NLP is used for. Some prominent examples are speech recognition, language detection, part-of-speech tagging, sentiment analysis, question answering, automatic summarization, text classification, character recognition, textual entailment, and many more.

NLP tasks can be divided into two main fields: analysis and generation tasks. Analysis tasks aim for analyzing existing text, while the objective of generation tasks is to generate new text. The analysis task can again be divided into syntactic, a language structure-based task, semantic, a meaning-based task, and pragmatic, a task of difficult open problems to solve \cite{ganegedara2018natural}. 
Figure~\ref{fig:NLP-Tasks-Taxonomy} illustrates the categorization of different NLP tasks as described by Ganegadara. 
\begin{figure}
    \centering
...
In contrast to sentiment analysis, ABSA is a more fine-grained task, consisting of two main subtasks, namely Aspect-Category Detection, and Aspect-Category Sentiment Classification. In the first (multi-label classification task \footnote{in single-label classification one sample in the data contains exactly one output label, while in the multi-label classification task, one sample can contain n output labels.}), a sentence is associated with a set of predefined single- and multi-word aspect terms (e.g., "operation performance", "battery"). The second task, the Aspect-Category Sentiment Classification, deals with the detection of sentiment associated with its aspect (e.g. "positive", "neutral", "negative") \cite{pavlopoulos2014aspect}. See Figure~\ref{fig:ABSA} for an illustration of the ABSA task.
...
\label{sec:bert}
In 2018, Google AI researchers achieved a breakthrough in NLP by introducing BERT, a deep contextual language representation model. BERT is an abbreviation for \textbf{B}idirectional \textbf{E}ncoder \textbf{R}epresentations from \textbf{T}ransformers \cite{devlin2018bert}. It is built on preceding innovations: \textbf{deep contextualized word representations} \cite{peters2018deep}, the \textbf{Transformer architecture} \cite{vaswani2017attention}, and \textbf{pre-training} on a language modeling task with subsequent \textbf{fine-tuning} on a downstream task \cite{radford2018improving, howard2018universal}.
...
% elaborate
BERT is a stack of transformer encoder layers that consist of a multi-head attention layer (heads), followed by a fully-connected neural network.
For every input token in a text sequence,( e.g., a sentence), each head computes key, value, and query vectors, which are then used to create a weighted representation. The outputs of all heads in the same layer are combined and run through a fully-connected layer. BERT uses the self-attention mechanism introduced with the Transformer model \cite{vaswani2017attention}.
%Fig xy exposes an example of how BERT uses the self-attention mechanism....

Similar to the Word2vec word embedding, BERT maps related words closely, but it is \textbf{context-sensitive}, meaning a different word vector is computed for a word if encountered in different contexts. In other terms, related words have a smaller distance in the word embedding than unrelated words, but can be discriminated by their context as BERT makes use of the information of the tokens in the sequence surrounding the word of interest. 
...
An important part of producing word embeddings is tokenization. As stated before, a word embeddings are feature vector representations of words. A feature vector is a list of values generated to represent one word, where the values are floating points and can be positive and negative. The distance of word embeddings displays the distance of word similarity. To give an example, the word embedding for the word "cat" could be
...
BERT large has a vocabulary of 30,522 tokens and 786 features in its embedding. For simplicity, Figure~\ref{fig:word_embedding} shows a table of only ten words tokens and five features.
...
\centering
    ...
    \begin{tikzpicture}[replace stretch/.style args={from #1 to #2 by #3}{%
        ...
        % tokenized object
        \node (0) [below of=vectorized, xshift=-6cm, yshift=-4.5cm, draw, thick, minimum height=1cm, minimum width=1.5cm] {\small [CLS]};
        \node (1) [right of=0, draw, xshift=0.6cm, thick, minimum height=1cm, minimum width=1.7cm] {\small I};
        \node (2) [right of=1, draw, xshift=0.7cm, thick, minimum height=1cm, minimum width=1.7cm] {\small like};
        \node (3) [right of=2, draw, xshift=0.7cm, thick, minimum height=1cm, minimum width=1.7cm] {\small straw};
        \node (4) [right of=3, draw, xshift=0.7cm, thick, minimum height=1cm, minimum width=1.7cm] {\small \#\#berries};
        \node (5) [right of=4, draw, xshift=0.7cm, thick, minimum height=1cm, minimum width=1.7cm] {\small [SEP]};
...
The segment embedding layer uses two indices 0 and 1 as vector representations to assign tokens to their corresponding input segment. The procedure is illustrated in Figure~\ref{fig:BERT_segmentEmbedding}.
...
BERT is a \textbf{bidirectional model}, which refers to the reading direction of the text input. While directional models read text sequentially, meaning left to right, or right to left \cite{peters2018deep, radford2018improving}, bidirectional models consider an entire sequence of words for their prediction, what allows them to learn the context of a word, based on those around it in the sequence.

\textbf{Pre-Training - MLM}

The first task to pre-train the BERT model is MLM. When solving the MLM task, 15\% of the words in a sequence are replaced with a [MASK] token. Based on the context of the remaining words, the model tries to predict the original value of the masked words, as illustrated in Figure~\ref{fig:BERT_MLM}. To do so, a classification layer is added on top of the encoder output, multiplying the output vector with the embedding matrix. The product of the calculation is represented in a vocabulary dimension. After that, the probability for each word in the vocabulary is calculated using the softmax.
...
Fine-tuning BERT for downstream tasks is computationally inexpensive, compared to pre-training. 
...
\begin{figure}
    ...
    \caption[BERT fine-tuning on a classification task]{Fine-tuning of BERT for a simple classification task detecting spam in E-Mails. Illustration from~\protect\cite{alammar2018illustrated}}
...
In 2013, Szegedy et al., were the first to discover the vulnerability of several machine learning models through adversarial examples: inputs crafted by adversaries with the intent of causing deep neural networks to misclassify  \cite{szegedy2013intriguing}. These carefully curated examples are correctly classified by a human observer but can fool a target model, raising serious concerns regarding the security of existing machine learning algorithms. 
...
The notation "adversarial example" is used in follow-up research to denote all kinds of perturbation samples in a general manner. In further research, Goodfellow et al. found that a wide variety of models with different architectures misclassify the same adversarial example, even when trained on different subsets of training data. Those examples are called transferable adversarial examples \cite{goodfellow2014explaining}. These findings indicate that adversarial examples pose a fundamental blind spot in machine learning algorithms.

Adversarial Examples can be found in all areas where DNNs are deployed. Most common attacks are conducted in the computer vision domain. In the following sections, we will introduce some of the basic concepts and present important work from the areas of adversarial examples in computer vision and NLP. Subsequently, we will evaluate the differences and discuss why attacks in computer vision cannot be applied one-to-one to NLP.

\subsubsection{Adversarial Examples in Computer Vision}
\label{sec:adversarial_exapmles_in_text}

The notation of evasion attacks has been established and describes adversarial attacks that exploit a DNN during inference time when the network is fed an adversarial example. 
...
A search method's aim is to find a perturbation that generates a successful adversarial example and satisfies the constraints. There have been different approaches introduced for this process over the years. The most popular can be categorized into:
    ...
    \textbf{a) Greedy with Word Importance Ranking}
        ...
        \textbf{Leave One Out Method for Important Word Detection}
            ...
            \DontPrintSemicolon
            \Indm 
            \KwInput{dataset $X$ with n sentences $s$, classify function}
            \KwResult{list of 0 ... n important words per sentence}
            \Indp
            initialize list important-words\;
            \For{sentence in X}{
            initialize list important-words-per-sentence\;
            original-classification = classify(sentence)\;
            \For{word in $s$}{
            leave-out = sentence without word\;
            modified-classification = classify(leave-out)\;
            }
            \If{original-classificaiton is not modified-classification}{
            add word to important-words-sentence\;
            }
            add important-words-sentence to important-words\;
            }
...
\textbf{\textit{RQ2: How does the robustness of the model behave when comparing the three different character level attacks?}}
...
\label{sec:datasets}
We conducted our experiment using \textbf{SemEval-2015 Task 12: Aspect Based Sentiment Analysis} dataset for the \textbf{laptop domain} \cite{pontiki2015semeval}. An aspect category is defined as a combination of an entity and an attribute type. An \textbf{entity} describes the reviewed entity (e.g. laptop), while an \textbf{attribute} describes a particular attribute of an entity (e.g. durability or quality).
To give an example:
The review \textit{"They sent it back with a huge crack in it and it still didn't work; and that was the fourth time I've sent it to them to get fixed"}, has the entity \textit{customer support} and the attribute \textit{quality}. The third part of the aspect is used to determine the \textbf{sentiment polarity} towards the entity and its attribute (\textit{positive, neutral, or negative}). The sentiment for the example given above is \textit{negative}.
For this thesis, we decided to use only one of the three provided domains, namely laptops. The laptop dataset comes with a standard training and testing data split and consists of 1,739 sentences in the training data and 761 sentences in the test data. Figure~\ref{fig:semeval} provides a table of entity and attributes labels used in the dataset. A sentence's sentiment polarity is labeled with the notation "POS", "NEU", or "NEG". Every entity can be combined with every attribute and every sentiment, theoretically resulting in 621 possible output classes. The dataset, however, contains a total of only 198 different output classes. The reason is that not all entities are combined with all attributes.
...
\hline
\textbf{Perturbation Method}                                      & \textbf{Leet Speak}           &\textbf{ Misspellings}         & \textbf{Punctuation}          \\ \hline
...
\textbf{Success Rate}                                             & \textbf{0.477599}             & \textbf{0.310192}             & \textbf{0.149511}             \\ \hline
\end{tabular}%
}
\caption[Result methods comparison]{Results of the three attack methods compared}
...
\textbf{Original Sentence} & It's {\color{green} wonderful} for computer gaming. & LAPTOP\#MISCELLANEOUS & POS \\ \hline
\textbf{Leet Speak} &
  It's {\color{red} w0nd3rfu1} for computer gaming. &
...
\textbf{Misspellings} &
  It's {\color{red}wonderfull} for computer gaming. &
  \begin{tabular}[c]{@{}l@{}}LAPTOP\#GENERAL\\ LAPTOP\#MISCELLANEOUS\end{tabular} &
  \begin{tabular}[c]{@{}l@{}}POS\\ POS\end{tabular} \\ \hline
\textbf{Punctuation} &
  It's {\color{red} wonderful,} for computer gaming. &
  \begin{tabular}[c]{@{}l@{}}LAPTOP\#GENERAL\\ LAPTOP\#MISCELLANEOUS\end{tabular} &
  \begin{tabular}[c]{@{}l@{}}POS\\ POS\end{tabular} \\ \hline
\end{tabular}%
}
\caption[Illustration of successful example in all attack methods]{Example sentence modified using the three attack methods Leet Speak, Misspellings, and Punctuation.}
\label{tab:ex_sentence_gaming}
\end{table}
To get an overview of how the different modification methods affected the algorithm's behavior regarding the individual output classes, we will now examine the label shifts.  We will break down the labels into their three components, entity, attribute, and sentiment, and show the differences per label part and attack. It is important to note that only those labels were included in the analysis, that occur in an original sentence where the perturbed sentence generated an adversarial example or added with the adversarial example. 
...
When only looking at the entities in Figure~\ref{fig:entity_typo}, we see that SOFTWARE, SUPPORT, and OS were removed most often by the Misspellings method, whereas LAPTOP and COMPANY are much more likely to be added than other entities.

\begin{figure}[H]
    \centering
    \includegraphics[ width = 1\textwidth]{img/entity_typo.png}
    \caption[Changes in Entity - Misspellings]{Changes in entities - Attack Method: Misspellings. Note that only results are displayed that have been changed and the ones that stay are hidden.}
...
\begin{figure}[H]
    ...
    \caption[Changes in Entity-Attribute - Misspellings]{Changes in entity-attribute pairs - Attack Method: Misspellings. Note that only results are displayed that have been changed and the ones that stay are hidden.}
    \label{fig:entity_attr_typo}
\end{figure}

Figure~\ref{fig:entity_attr_sent_typo} shows that positive reviews are more likely to be not be classified as such through Misspellings perturbations, while more reviews are falsely classified to be negative, while they are not.

\begin{figure}[H]
    \centering
    \includegraphics[ width = 1\textwidth]{img/entity_asp_sent_typo.png}
    \caption[Changes in Entity-Attribute-Sentiment - Misspellings]{Changes in entity-attribute sentiment sets - Attack Method: Misspellings. Note that only results are displayed that have been changed and the ones that stay are hidden.}
...
\section{Discussion}
In this chapter, we will delve into the meaning of our results and show how it relates to the discussed literature. We will reiterate our research questions and concisely summarize our major findings, to provide a clear statement of the overall result.  

\textbf{\textit{RQ1: Is the pre-trained BERT model, fine-tuned on the ABSA task robust against input level adversarial examples?}}

Through our experiments, we were able to show that the BERT model can be fooled by input modifications of the character level and is therefore \textbf{not robust against input level adversarial examples.} All three experiments, in which different attack methods were used, were successful and could cause the classifier to predict incorrect results. Thus, we could show that the robustness of the BERT model can easily be affected by attacks, even in the black-box setting, without further knowledge about the model's internal parameters or output label confidence levels.
We were able to achieve a changed prediction in certain input sentences by applying \textit{any} of the three attacking methods.

\textbf{\textit{RQ2: How does the robustness of the model behave when comparing the three different character level attacks?}}

The comparison of the three attack methods indicates that \textbf{there is a significant difference between the different method applied.} The most successful method is Leet Speak, with which we were able to achieve a success rate of 47.8\%. The Misspellings method resulted in a success rate of 31\%, and through Punctuation modification, we achieved a success rate of 15\%. While the experiments on Leet Speak produced an incorrect classification of each entity, we cannot detect any equally robust or susceptible entities to all three methods. Interestingly, certain classification labels were seemingly very susceptible to one of the three attack methods, while this was not the case for the two other attack methods. When analyzing the changed sentiment predictions, we found that for the punctuation method, positive reviews were more likely to be classified as such, while by applying the other two methods, the same reviews were more often falsely classified as negative when they were not. 
