...
A DNN consists of neurons, that receive an input $x$, process it and generate an output. The output is either a final output $y$ or fed to 
...
\todo{explain data transformations - neurons - weights - feed-forward} The model improves by consciously adjusting the model in the process of transforming the input $x$ to the output $y$.  This process runs until the mapping from input to output no longer improves. 
...
% Text Node
\draw (98,220.8) node [anchor=north west][inner sep=0.75pt]  [font=\tiny]  {${\textstyle Input\ Layer\ L_{1}}$};
% Text Node
\draw (179,219.8) node [anchor=north west][inner sep=0.75pt]  [font=\tiny]  {$Hidden\ Layers\ L_{2} \ ...\ L_{5}_{\ }$};
% Text Node
