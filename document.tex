...
To prevent this happenings, online platform providers prohibit the spreading of hate speech and implement DNN models to detect hateful content \cite{badjatiya2017deep}.
...
Given its increasing use in security-sensitive applications such as sentiment analysis \cite{prabowo2009sentiment} and toxic content detection \cite{singhania20173han}, the high reliance on the BERT model for such tasks is highly concerning, since the vulnerability of the model is still not yet fully researched.
...
Our goal is to point out that commonly used state-of-the-art models for NLP are vulnerable against input level attacks and we should not rely solely on those automated algorithms for security and safety relevant tasks, such as, e.g. hate speech detection.
...
BERT uses the self-attention mechanism introduced with the Transformer model \cite{vaswani2017attention} and described inChapter~\ref{sec:attention}. 
