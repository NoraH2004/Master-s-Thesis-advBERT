...
<0.4125, -1.6098, 0.6047, ..., -1.4257, -1.231>.
...
The purpose of the token embedding layer is to tokenize the input (e.g. "I like strawberries") and transform it into a vector representation of fixed dimensions (see Figure~\ref{fig:BERT_tokenEmbedding}). In case of BERT base, each word is represented as a 768-dimensional vector.
\begin{figure}
    \centering
...
Therefore we decided on the described three attack methods. 
...
Let x = [x1, x2, … xn] be the input sentence represented as a list of words with the predicted probability of a label y, and y = argmaxy%^% p(%^%yjx). The important word is defined as g(xi) = p(yjx) − p(yjx−i).
...
\section{Limitation & further Research}
