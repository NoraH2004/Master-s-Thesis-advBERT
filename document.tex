...
The \textbf{Search Method} used in this thesis is called Leave-One-Out (LOO) and explained in section ~\ref{sec:iwd_method}.
...
For fine-tuning the pre-trained BERT model on the ABSA task, we, again, used 32 bit floating point computations, the Adam optimizer and set the learning rate to 3 $*$ 10$^{-5}$. We used a batch size of 32 and fine-tuned it for 20 epochs. 
...
We apply this formula for every word token $x$ in a input sequence to determine all words influential to the prediction. Therefore, one input sequence can contain $[0,1, \dots n]$ important words. 
...
\subsection{Pre-Processing and Important Word Detection todo}
...
\subsection{Attacks todo}
\label{sec:attacks_exp}

\subsection{Evaluation todo}
    \label{sec:evaluation_meth}
~~~vage~~
