...
% this solution uses the changepage package
% TODO replace hardcoded sources with correct citation
\begin{table}
\begin{adjustwidth}{-3cm}{}
\small
\begin{tabular}{p{2cm}|p{2cm}p{4cm}p{3cm}p{5cm}}
\toprule
...
\midrule
\textbf{Tan et al., 2020}&Greedy search &Inflection Word Swap &Minimum BLEU Score & \\
\midrule
\textbf{Pruthi et al., 2019} &Greedy search &Character insertion, deletion, neighboring swap, keyboard-based character swap &Untargeted classification &minimum word length, maximum number of words perturbed \\
\midrule
\textbf{Zang et al., 2020} &Particle Swarm Optimization&HowNet Word Swap &Untargeted classification & \\
\midrule
\textbf{Ren et al., 2019} &Greedy-WIR (saliency) &WordNet-based synonym swap &Untargeted classification & \\
\midrule
\textbf{Cheng et al., 2018} &Greedy-WIR &Counter-fitted word embedding swap &Non-overlapping output & \\
\midrule
\textbf{Li et al, 2019} &Greedy-WIR &Character insertion, deletion, substitution, neighboring swap&Untargeted Classification &USE sentence encoding cosine similarity \\
\midrule
...
For fine-tuning the pre-trained BERT model on the ABSA task, we used the Ranger optimizer and set the learning rate to $3 * 10^{-5}$. \todo{explain ranger} We used a batch size of 32 and fine-tuned it for 20 epochs and random initialization. 
